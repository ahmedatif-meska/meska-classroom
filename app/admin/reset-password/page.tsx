...
export default async function AdminResetPassword({
  ...
  // A valid admin recovery session must be present (established by the confirm route).
  // Absent it — or with ?error=link from a rejected link — show the invalid-link state.
  ...
  if (error !== "link") {
    ...
    validSession = assertAdminSession(user ? { user } : null).ok;
  }
