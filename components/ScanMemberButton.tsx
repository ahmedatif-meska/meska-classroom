      ...
      try {
        ...
        await scanner.start(
          ...
          (decoded: string) => {
            ...
            // Stop the camera, then navigate via a FULL-PAGE load (handled in
            // navigateIfMember). A client-side router.push would unmount this page
            // while html5-qrcode is still tearing down its <video>, and that race
            // throws into the error boundary.
            const go = () => navigateIfMember(decoded);
            ...
            {error ? (
              <div className="mt-4">
                ...
                <button
                  ...
                  className="mt-3 w-full rounded-full border border-brand px-5 py-2.5 text-sm font-semibold text-brand hover:bg-brand/5 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand"
                >
